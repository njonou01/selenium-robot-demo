...
/**
 * Intercepte, par tissage AspectJ, toute methode annotee {@link Workflow} (le pointcut reference
 * son chemin qualifie complet et doit etre mis a jour si l'annotation change de package). Convertit
 * chaque appel en {@link TestStep} du rapport seleniumRobot: nom/description/resultat attendu
 * interpoles depuis les arguments reels de la methode (${nom} -> valeur), gestion de
 * l'imbrication parent/enfant quand un workflow en appelle un autre, capture d'ecran sur l'etape
 * racine, propagation de l'exception d'origine si l'execution echoue.
 */
@Aspect
public class WorkflowAspect {
    ...
    @Around(
            "execution(@com.example.seleniumdemo.reporting.Workflow * *(..)) "
                    + "&& @annotation(workflow)"
    )
        ...
        try {
            ...
            if (rootStep) {
                try {
                    ...
                    // ne bloque jamais le workflow pour un problème de snapshot
                }
            }
